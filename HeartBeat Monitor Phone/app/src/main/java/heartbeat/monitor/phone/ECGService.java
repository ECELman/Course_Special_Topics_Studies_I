...
public class ECGService
{
	...
	private int[][] window = new int[1024][10];
	private int[] window_count = new int[1024];
	...
	public synchronized void setState(int state) {
		...
		mHandler.obtainMessage(Monitor.MESSAGE_KY_STATE , state, -1).sendToTarget();
	}
	...
	public void DataHandler (byte[] Data, int now_patinet_id_data, int choose)
	{
//		if (D) Log.d(TAG, "DataHandler");
		...
		for(int i=0; i < Data.length ; i++ )
		{
			...
			if (iRawData[now_patinet_id_data] < RawSize32)
			{
				Raw_Buffer[now_patinet_id_data][iRawBuffer[now_patinet_id_data]] = Data[i];
				iRawData[now_patinet_id_data]++;
				iRawBuffer[now_patinet_id_data]++;
				iDataEnd = i;

				if(iRawBuffer[now_patinet_id_data]==RawBufferSize)
				{
					// Notice (A)
					byte [] rawData = new byte[RawBufferSize];
					System.arraycopy(Raw_Buffer[now_patinet_id_data], 0, rawData, 0, RawBufferSize);

					// Send the obtained bytes to the UI Activity
					// arg1-> length, arg2-> -1, obj->buffer
					int state = stateCheck(rawData, now_patinet_id_data, first[now_patinet_id_data]);

					Message msg = mHandler.obtainMessage(Monitor.MESSAGE_WARN);
					Bundle bundle = new Bundle();
					bundle.putString(Monitor.WARN_TYPE, String.valueOf(state));
					bundle.putString(Monitor.Patient_Id, Integer.toString(now_patinet_id_data));
					msg.setData(bundle);
					mHandler.sendMessage(msg);

					if(now_patinet_id_data==choose){
						mHandler.obtainMessage(Monitor.MESSAGE_RAW, RawBufferSize, -1, rawData)
								.sendToTarget();
					}

					msg = mHandler.obtainMessage(Monitor.MESSAGE_UPDATE);
					bundle = new Bundle();
					bundle.putString(Monitor.UPDATE_STRING,  now_patinet_id_data + "," + "ECG" + " " + BytesToString(rawData));
					msg.setData(bundle);
					mHandler.sendMessage(msg);

					iRawBuffer[now_patinet_id_data] = 0;
				}
			...
			else if(Data[i] == 0x0D)
			{
				...
				InfoHandler(Info_Buffer, now_patinet_id_data);
	...
	public int stateCheck(byte[] Data, int now_patinet_id_data, int first){
		...
		if(Ratio_RR_count[now_patinet_id_data]<=0){
			Ratio_RR_count[now_patinet_id_data]=-1;
			average_RR_count[now_patinet_id_data]=100;
		}
		...
		if(max_point!=0 && max_point+8<Data.length){
			...
			if(max_number>130)
				average_R[now_patinet_id_data] = (float)(average_R[now_patinet_id_data]*0.75 + max_number*0.25);
			if(max_number>average_R[now_patinet_id_data]-10){
				R = max_point;

				if(Ratio_RR_count[now_patinet_id_data]>0){
					Ratio_RR_count[now_patinet_id_data] = Ratio_RR_count[now_patinet_id_data]+max_point;
					if(Ratio_RR_count[now_patinet_id_data]<280){
						Ratio_RR[now_patinet_id_data] = (float)Ratio_RR_count[now_patinet_id_data]/average_RR_count[now_patinet_id_data];
						average_RR_count[now_patinet_id_data] = (float) (average_RR_count[now_patinet_id_data] * 0.75 + (float)Ratio_RR_count[now_patinet_id_data] * 0.25);
					}
				}
				Ratio_RR_count[now_patinet_id_data] = 64-max_point;
				...
				if(Ratio_RR_count[now_patinet_id_data]>0)
					Ratio_RR_count[now_patinet_id_data] = Ratio_RR_count[now_patinet_id_data]+64;
			}
		}
		else{
			if(Ratio_RR_count[now_patinet_id_data]>0)
		...
		if(Q>0 && R>0){
			H_QR[now_patinet_id_data] = ((float)value[R] - (float)value[Q])*6/256;
			Slope_QR[now_patinet_id_data] = H_QR[now_patinet_id_data]/(((float)R - (float)Q)*4);
		}
		if(R>0 && S>0){
			H_RS[now_patinet_id_data] = ((float)value[R] - (float)value[S])*6/256;
			Slope_RS[now_patinet_id_data] = H_RS[now_patinet_id_data]/(((float)S - (float)R)*4);
		}
		if(Q>0 && S>0)
			QRS_dur[now_patinet_id_data] = ((float)S - (float)Q)*4;
		if(Q>0 && R>0 && S>0){
			if(H_QR[now_patinet_id_data]>Slope_RS[now_patinet_id_data])
				Area_QRS[now_patinet_id_data] = H_QR[now_patinet_id_data]*QRS_dur[now_patinet_id_data]/2;
			else
				Area_QRS[now_patinet_id_data] = H_RS[now_patinet_id_data]*QRS_dur[now_patinet_id_data]/2;
		}
		if(Q>0 && T_prime>0)
			QTP_int[now_patinet_id_data] = ((float)T_prime - (float)Q)*4;
		if(R>0 && S>0 && T_prime>0){
			Area_RST[now_patinet_id_data] = (((float)value[R] - (float)value[S])*6/256)*(((float)T_prime - (float)R)*4)/2;
		}

		now_state = UNKNOWN;
		if(H_QR[now_patinet_id_data]>0 && H_RS[now_patinet_id_data]>0 && QRS_dur[now_patinet_id_data]>0 && QTP_int[now_patinet_id_data]>0 && Ratio_RR[now_patinet_id_data]>0 && Slope_QR[now_patinet_id_data]>0 && Slope_RS[now_patinet_id_data]>0 && Area_QRS[now_patinet_id_data]>0 && Area_RST[now_patinet_id_data]>0){

			for(int i=0; i<50; i++){
				a = (QRS_dur[now_patinet_id_data]-CodeBook_4[i][0])*(QRS_dur[now_patinet_id_data]-CodeBook_4[i][0])
						+ (QTP_int[now_patinet_id_data]-CodeBook_4[i][1])*(QTP_int[now_patinet_id_data]-CodeBook_4[i][1])
						+ (Area_QRS[now_patinet_id_data]-CodeBook_4[i][2])*(Area_QRS[now_patinet_id_data]-CodeBook_4[i][2])
						+ (Area_RST[now_patinet_id_data]-CodeBook_4[i][3])*(Area_RST[now_patinet_id_data]-CodeBook_4[i][3]);
				if(a<min){
					min = a;
					vector_value = i;
				}
			...
			min = 10000000;
			for(int i=0; i<50; i++){
				a = (H_QR[now_patinet_id_data]-CodeBook_8[i][0])*(H_QR[now_patinet_id_data]-CodeBook_8[i][0])
						+ (H_RS[now_patinet_id_data]-CodeBook_8[i][1])*(H_RS[now_patinet_id_data]-CodeBook_8[i][1])
						+ (QRS_dur[now_patinet_id_data]-CodeBook_8[i][2])*(QRS_dur[now_patinet_id_data]-CodeBook_8[i][2])
						+ (QTP_int[now_patinet_id_data]-CodeBook_8[i][3])*(QTP_int[now_patinet_id_data]-CodeBook_8[i][3])
						+ (Slope_QR[now_patinet_id_data]-CodeBook_8[i][4])*(Slope_QR[now_patinet_id_data]-CodeBook_8[i][4])
						+ (Slope_RS[now_patinet_id_data]-CodeBook_8[i][5])*(Slope_RS[now_patinet_id_data]-CodeBook_8[i][5])
						+ (Area_QRS[now_patinet_id_data]-CodeBook_8[i][6])*(Area_QRS[now_patinet_id_data]-CodeBook_8[i][6])
						+ (Area_RST[now_patinet_id_data]-CodeBook_8[i][7])*(Area_RST[now_patinet_id_data]-CodeBook_8[i][7]);
				if(a<min){
					min = a;
					vector_value = i;
				}
			...
			if(first==1){
				if(Ratio_RR[now_patinet_id_data]>0.88){
					for(int i=0; i<=2; i++){
						for(int j=0; j<=4; j++){
							if(j<=2){
								probability[i][j]=(double)PI_first[i]*(double)B_code[i][previous_vector_8[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_8];
							}
							else{
								probability[i][j]=(double)PI_first[i]*(double)B_code[i][previous_vector_8[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_4];
							}
						}
					}
					max_value=0;
					...
					for(int i=3; i<=4; i++){
						for(int j=0; j<=4; j++){
							if(j<=2){
								probability[i][j]=(double)PI_first[i]*(double)B_code[i][previous_vector_4[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_8];
							}
							else{
								probability[i][j]=(double)PI_first[i]*(double)B_code[i][previous_vector_4[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_4];
							}
						}
					}
					max_value=0;
				...
				if(Ratio_RR[now_patinet_id_data]>0.88){
					for(int i=0; i<=2; i++){
						for(int j=0; j<=4; j++){
							if(j<=2){
								probability[i][j]=(double)A_state[previous_state[now_patinet_id_data]][i]*(double)B_code[i][previous_vector_8[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_8];
							}
							else{
								probability[i][j]=(double)A_state[previous_state[now_patinet_id_data]][i]*(double)B_code[i][previous_vector_8[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_4];
							}
						}
					}
					max_value=0;
					...
					for(int i=3; i<=4; i++){
						for(int j=0; j<=4; j++){
							if(j<=2){
								probability[i][j]=(double)A_state[previous_state[now_patinet_id_data]][i]*(double)B_code[i][previous_vector_4[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_8];
							}
							else{
								probability[i][j]=(double)A_state[previous_state[now_patinet_id_data]][i]*(double)B_code[i][previous_vector_4[now_patinet_id_data]]*(double)A_state[i][j]*(double)B_code[j][vector_4];
							}
						}
					}
					max_value=0;
			...
			previous_vector_4[now_patinet_id_data] = vector_4;
			previous_vector_8[now_patinet_id_data] = vector_8;
			previous_state[now_patinet_id_data] = now_state;
		}

		if(now_state<5){

			State_num[now_patinet_id_data][now_state]++;
			String A = Integer.toString(State_num[now_patinet_id_data][0]) + " " + Integer.toString(State_num[now_patinet_id_data][1]) + " " + Integer.toString(State_num[now_patinet_id_data][2]) + " " + Integer.toString(State_num[now_patinet_id_data][3]) + " " + Integer.toString(State_num[now_patinet_id_data][4]) + "    ";

			window[now_patinet_id_data][window_count[now_patinet_id_data]]=now_state;
			window_count[now_patinet_id_data]=(window_count[now_patinet_id_data]+1)%10;
			...
			for(int i=0; i<10; i++){
				s[window[now_patinet_id_data][i]]++;
			}

			if(s[3]>=3 && (s[0]+s[3])>=8){
				State_num_2[now_patinet_id_data][3]++;
			}
			if(s[4]>=3 && (s[0]+s[4])>=8){
				State_num_2[now_patinet_id_data][4]++;
			}
			if(s[0]>=5){
				State_num_2[now_patinet_id_data][0]++;
			}
			if(s[1]>=5){
				State_num_2[now_patinet_id_data][1]++;
			}
			if(s[2]>=5){
				State_num_2[now_patinet_id_data][2]++;
			}

			String B = Integer.toString(State_num_2[now_patinet_id_data][0]) + " " + Integer.toString(State_num_2[now_patinet_id_data][1]) + " " + Integer.toString(State_num_2[now_patinet_id_data][2]) + " " + Integer.toString(State_num_2[now_patinet_id_data][3]) + " " + Integer.toString(State_num_2[now_patinet_id_data][4]) + "    ";
		}
		return now_state;
	}

	public void InfoHandler (byte[] Info, int now_patinet_id_data)
	{
		...
		{
			iRawData[now_patinet_id_data] = 0;
		}else
		{
			// Send a Info message back to the Activity
			Message msg = mHandler.obtainMessage(Monitor.MESSAGE_INFO);
			Bundle bundle = new Bundle();
			bundle.putString(Monitor.KY_INFO, InfoStr);
			bundle.putString(Monitor.Patient_Id, Integer.toString(now_patinet_id_data));
			msg.setData(bundle);
			mHandler.sendMessage(msg);
		}
 ...
 * 因此 為避免資料錯誤，不能直接傳送buffer，而是從buffer中
 * 取出要傳送的區段，另new出一個byte[]實體作為傳送的物件。
 */
